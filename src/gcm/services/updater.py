...
Running from source (``python -m gcm.app``) never self-updates -- only
``check_latest_release`` is useful there, so the caller can point the user
at the release page instead of offering a download.

The network call is a plain blocking ``urllib.request`` call; UI callers
(which run on qasync's event loop) should invoke it via
...
GITHUB_REPO = "mediaswing/graphical-cloud-manager"
APP_NAME = "GraphicalCloudManager"
...
def _install_root() -> str:
    ...
    if sys.platform == "darwin":
        # exe is .../GraphicalCloudManager.app/Contents/MacOS/GraphicalCloudManager
...
def perform_self_update(release: ReleaseInfo) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix=f"{APP_NAME}-update-")
